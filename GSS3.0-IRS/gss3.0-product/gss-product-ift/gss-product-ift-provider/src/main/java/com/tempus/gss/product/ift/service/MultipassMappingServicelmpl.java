 ...
 */
@Service
public class MultipassMappingServicelmpl implements IMultipassMappingService {
    ...
    public Page<QueryIBEDetail> query(Page<QueryIBEDetail> page, RequestWithActor<FlightQueryRequest> flightQueryRequest) {
        ...
        shoppingSeg.setDestinationLocation(flightQuery.getTransfers().get(0));
     ...
     * @return
     */
    private void mappingPriceSpec(QueryIBEDetail queryIBEDetail, String customerTypeNo, Agent agent) {
        ...
        if (2 == priceSpec.getGoDep()) {//出票航自承运第一个航段起点
            for (Flight flight : queryIBEDetail.getFlights()) {
                if (flight.getDirection().equals("go")) {
                    if (flight.getAirline().equals(queryIBEDetail.getTicketAirline()) && (flightNum >= flight.getFlightNum()
                            || flightNum == 0)) {
                        goDep = flight.getDepAirport();
                        direction = flight.getDirection();
                        flightNum = flight.getFlightNum();
        ...
        if (2 == priceSpec.getMiddle()) {//去程/回程出票航的最远点
            for (Flight flight : queryIBEDetail.getFlights()) {
                if (flight.getDirection().equals("go")) {
                    if (flight.getAirline().equals(queryIBEDetail.getTicketAirline()) && direction
                            .equals(flight.getDirection()) && (flightNum <= flight.getFlightNum() || flightNum == 0)) {
                        goArr = flight.getArrAirport();
                        backDep = flight.getArrAirport();
                        flightNum = flight.getFlightNum();
                    }
        ...
        if (3 == priceSpec.getBackArr()) {// 最后一个跨国段的终点
            for (Flight flight : queryIBEDetail.getFlights()) {
                ...
                String arrCountry = arrAirport.getCountryCode();
                if (flight.getDirection().equals("back")) {
                    if (!direction.equals(flight.getDirection()) && !depCountry.equals(arrCountry) && (
                            flightNum <= flight.getFlightNum() || flightNum == 0)) {
        ...
        if (goDep.equals(backArr)) {
            Airport depAirport = airportService.queryAirportByCode(goDep, "I");
            if (null == depAirport) {
                depAirport = airportService.queryAirportByCode(goDep, "D");
            }
            Airport arrAirport = airportService.queryAirportByCode(backArr, "I");
            if (null == arrAirport) {
                arrAirport = airportService.queryAirportByCode(backArr, "D");
            }
        ...
        for (PassengerTypePricesTotal passengerTypePricesTotal : queryIBEDetail.getCabinsPricesTotalses().get(0).getPassengerTypePricesTotals()) {
            ...
            if (passengerTypePricesTotal.getAward()) {//如果价格记奖才去匹配政策
                policyMap = mappingPolicy(goDep, goArr, backArr, queryIBEDetail.getTicketAirline(), passengerType,
                        goFlights, backFlights, customerTypeNo, agent);
            }
            ...
            if (2 == queryIBEDetail.getLegType()) {//往返政策要根据总则的计算方式计算
                ...
                BigDecimal salePrice = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, passengerTypePricesTotal.getFare(), passengerTypePricesTotal.getTax(), passengerTypePricesTotal.getFare().subtract(Qvalue), goFlights, "go");
                passengerTypePricesTotal.setSalePrice(salePrice.setScale(0, BigDecimal.ROUND_UP));
            }
            if (null != policyMap) {//有政策获取政策中的代理费率，返点，手续费
                ...
                passengerTypePricesTotal.setAwardPrice((BigDecimal) policyMap.get("awardPrice"));
        ...
        if (null != backArr && !"".equals(backArr)) {//取返程政策
            ...
            for (int i = backPolicyList.size() - 1; i >= 0; i--) {//根据限制条件去除政策
                ...
                if (!checkPolicyDate(policy, backFlight, true) ||
                        ...
                        !checkPolicyOther(policy, backFlights)) {
    ...
    private List<Map> mappingCabins(List<Policy> policys, Flight goflight, String customerTypeNo) {
        ...
        for (Policy policy : policys) {
            ...
            for (Cabin cabin : policy.getCabinList()) {
                if (cabin.getCabin().contains(goflight.getFlightCabinPriceVos().get(0).getCabin()) && false != goflight.getFlightCabinPriceVos().get(0).isAward()) {
         ...
         */
        private List<QueryIBEDetail> shoppingOutPutConvertQueryIBEDetails(ShoppingOutPut shoppingOutPut,Integer legType) throws ParseException {
            ...
            for (ShoppingOD shoppingOD : availableJourney.getOdOption()) {
                ...
                if (null != shoppingOD.getFareBases()) {
                ...
                for (ShoppingFlight shoppingFlight : shoppingOD.getFlight()) {
                    ...
                    for (PassengerTypePricesTotal passengerTypePricesTotal : passengerTypePricesTotals) {
                        if (null != shoppingFlight.getPsgerFltInfos()) {
    ...
    private Map<String, String> getNucByFareLinear(String fareLinear, String arr, String arrCity, String dep, String depCity, String fareBasis, Integer legType) {
        ...
        if (fareLinear.contains("/it") || "".equals(fareLinear)) {
            map.put("Q", "0");
                ...
                if (reTpm > 0) {//不记奖航段计算价格
                    if (null != formula01Para.getRegionList() && formula01Para.getRegionList().size() > 0) {
                        ...
                        for (Formula01Region formula01Region : formula01Para.getRegionList()) {
                            if (formula01Region.getLow().compareTo(new BigDecimal(reTpm / tpmTotal)) < 1 &&
                                    formula01Region.getHing().compareTo(new BigDecimal(reTpm / tpmTotal)) > 0) {
    ...
    private BigDecimal getCombinationPrice(PassengerTypePricesTotal passengerTypePricesTotal, QueryIBEDetail queryIBEDetail, PriceSpec priceSpec, Map policyMap, int type) {
        ...
        if (null == policyMap) {//往返没政策计算票价
            BigDecimal awardGoPrice = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, passengerTypePricesTotal.getFare().divide(new BigDecimal(2)),
                    passengerTypePricesTotal.getTax(), new BigDecimal(0), queryIBEDetail.getFlights(), "go");
            BigDecimal awardBackPrice = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, passengerTypePricesTotal.getFare().divide(new BigDecimal(2)),
                    new BigDecimal(0), new BigDecimal(0), queryIBEDetail.getFlights(), "back");
            return awardGoPrice.add(awardBackPrice);
        } else {//往返有政策计算票价
            Map<String, String> uncMap = getNucByFareLinear(passengerTypePricesTotal.getFareLinear(),
                    queryIBEDetail.getGoArrAirport(), arrCity, queryIBEDetail.getGoDepAirport(), depCity, passengerTypePricesTotal.getFareBasis(), queryIBEDetail.getLegType());
            if (1 == type) {//1（往返程价格分开算）
                BigDecimal Qvalue = new BigDecimal(0);
                if (null != uncMap.get("Q")) {//获取Q值
                    Qvalue = new BigDecimal((String) uncMap.get("Q")).multiply(new BigDecimal((String) uncMap.get("roe")));
                }
                BigDecimal awardGoFare = new BigDecimal((String) uncMap.get("goNuc"))
                        .multiply(new BigDecimal((String) uncMap.get("roe")));
                BigDecimal awardBackFare = new BigDecimal((String) uncMap.get("backNuc"))
                        .multiply(new BigDecimal((String) uncMap.get("roe")));
                ...
                BigDecimal awardGoPrice = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, awardGoFare.add(Qvalue),
                        passengerTypePricesTotal.getTax(), awardGoFare, queryIBEDetail.getFlights(), "go");
                BigDecimal awardBackPrice = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, awardBackFare,
                        new BigDecimal(0), awardBackFare, queryIBEDetail.getFlights(), "back");
                return awardGoPrice.add(awardBackPrice);
            }
            if (2 == type) {// 2（往返程价格1/2）
                ...
                BigDecimal awardGoPrice2 = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, awardGoFare2,
                        passengerTypePricesTotal.getTax(), awardGoFare2.subtract(Qvalue2.divide(new BigDecimal(2))),
                        queryIBEDetail.getFlights(), "go");
                BigDecimal awardBackPrice2 = getfare(policyMap, priceSpec.getFormula(), queryIBEDetail, awardBackFare2,
                        new BigDecimal(0), awardBackFare2.subtract(Qvalue2.divide(new BigDecimal(2))),
                        queryIBEDetail.getFlights(), "back");
    ...
    private void insertBatchVoyages(QueryIBEDetail queryIBEDetail, String type) {
        ...
        if (3 == queryIBEDetail.getLegType().intValue()) {
            voyageLowestPrice.setDepartDate(fl.format(queryIBEDetail.getGoDepTime()) + "/" + fl.format(queryIBEDetail.getGoArrTime()));
        }
    ...
    public BigDecimal getProfitByAgent(Agent agent) {
        ...
        if (profits != null && profits.size() > 0) {
            for (Profit bean : profits) {
                if (bean.getCustomerNo().equals(agent.getNum())) {
