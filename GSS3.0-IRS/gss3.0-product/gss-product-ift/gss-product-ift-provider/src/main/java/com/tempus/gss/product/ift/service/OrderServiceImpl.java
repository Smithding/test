...
public class OrderServiceImpl implements IOrderService {
    ...
    public boolean issuing(RequestWithActor<PassengerListVo> requestWithActor) {
        ...
        try {
            ...
            /* 创建操作日志 */
            Long transactionOrderNo = buyOrderList.get(0).getTransationOrderNo();
            saveLog(agent, saleOrderNo, logstr, transactionOrderNo);
            log.info("出单操作成功");
            //销售员订单数量减一
    ...
    public List<QueryByPnr> queryByPnr(Agent agent, String pnr) {
        ...
        for (Pnr pnrS : pnrList) {
            ...
            if (ext == null)
                ext = saleOrderExtDao.queryByPnrNo(pnrS.getPnrNo());
            if (ext != null) {
